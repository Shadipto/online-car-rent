document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll("[data-category-bar] a[data-type]")
    .forEach((link) => {
...
function validateProfileForm(form) {
  ...
  if (newPassword !== "") {
    if (form.elements.current_password.value === "") {
      showFieldError(
        form.elements.current_password,
...
function renderCarCard(car) {
  const image = car.image_path
    ? `<img src="${escapeHtml(window.location.origin + "/online-car-rent/public/" + car.image_path)}" alt="${escapeHtml(car.name)}">`
